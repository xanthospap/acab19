...
static_columns = ['iso_code','continent','location','tests_units','population','population_density','median_age','aged_65_older','aged_70_older','gdp_per_capita','extreme_poverty','cardiovasc_death_rate','diabetes_prevalence','female_smokers','male_smokers','handwashing_facilities','hospital_beds_per_thousand','life_expectancy','human_development_index']
...
def get_csv(filename=None):
    ...
    print('Downloading dataset file {:} to {:} ...'.format(COVID_URI, filename), end='')
    with urllib.request.urlopen(COVID_URI) as response, open(
...
def countries_in_continent(df, continent):
    """
    """
    return list(flatten(df.loc[df['continent']==continent, ['location']].drop_duplicates(subset=['location'], keep='last').values))
...
def get_cols_by_location(df, **kwargs):
    ...
    if columns == []:
        return df[df['location'].isin(country_list) | df['continent'].isin(continent_list)]
    else:
        if columns != []:
            ## drop duplicate locations if we only want 'static' columns
            if len([ x for x in columns if x in static_columns ]) == len(columns):
                return df.loc[df['location'].isin(country_list) | df['continent'].isin(continent_list), columns].drop_duplicates(subset=['location'], keep='last')
        return df.loc[df['location'].isin(country_list) | df['continent'].isin(continent_list), columns]
...
def create_scatter(df, **kwargs):
    ...
    if x_axis != 'date':
        print('[ERROR] Only \'date\' is allowed as x-axis when creating a dynamic (scatter) plot.', file=sys.stderr)
        print('[ERROR] You requested x-axis to be \'{:}\''.format(x_axis), file=sys.stderr)
    ...
    if 'continents' in kwargs:
        ...
        ndf = get_cols_by_location(df, continents=kwargs['continents'], columns=kw_columns)
    ...
    if 'countries' in kwargs:
        ndf = get_cols_by_location(df, countries=kwargs['countries'], columns=kwargs['columns']+['location'])
        for country in kwargs['countries']:
            ...
            #except:
            #    print('[WARNING] Ommiting country {:}; no data to plot for {:}/{:}'.format(c, x_axis, y_axis))
    plt.legend(legend)
    plt.title('Coivd-19 {:} Vs {:}; data source:\n{:}'.format(x_axis, y_axis, COVID_URI))
    plt.grid(True)
    plt.show()
...
def create_bar(df, **kwargs):
    ...
    frames = []
    if 'continents' in kwargs:
        frames.append(get_cols_by_location(df, continents=kwargs['continents'], columns=kwargs['columns']))
    if 'countries' in kwargs:
        frames.append(get_cols_by_location(df, countries=kwargs['countries'], columns=kwargs['columns']))
    ndf = pandas.concat(frames)
    labels = ndf['location']
    x = np.arange(len(labels))
    width = 0.35e0
    fig, ax = plt.subplots()
    bars = [ ax.bar(x-width/2, ndf[c].values, label=c) for c in kwargs['columns'][1:] ]
    ax.set_xticks(x)
...
parser.add_argument('--print-columns',
                    dest='print_columns',
                    action='store_true',
                    help='Print columns of dataframe, aka the recorded, raw data descrition of the dataset values.')
parser.add_argument(
    '--countries',
    metavar='COUNTRIES',
    dest='countries',
...
if args.print_columns:
  ...
  for idx, key in enumerate(df.columns): print('\t[{:3d}] -> {:}'.format(idx, key))
  sys.exit(0)
...
if args.x_axis in geo_columns and args.y_axis in static_columns:
    create_bar(df, countries=country_list, continents=continent_list, columns=[args.x_axis, args.y_axis])
else:
    create_scatter(df, countries=country_list, continents=continent_list, columns=[args.x_axis, args.y_axis])
